...
{
    public interface IHasanoglanIsikKureModbusService
    {
    ...
    public class HasanoglanIsikKureModbusService : IHasanoglanIsikKureModbusService, IDisposable
    {
        private readonly HasanoglanIsikKureSettings _settings;
        private readonly ILogger<HasanoglanIsikKureModbusService> _logger;
        ...
        public HasanoglanIsikKureModbusService(ILogger<HasanoglanIsikKureModbusService> logger)
        {
            _logger = logger;
            _settings = new HasanoglanIsikKureSettings();
        }
        ...
        private async Task<bool> EnsureConnectedAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Hasanoğlan IŞIKKURE Modbus bağlantısı başarılı: {Ip}:{Port}", _settings.IpAddress, _settings.Port);
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Hasanoğlan IŞIKKURE Modbus bağlantı hatası");
                return false;
            }
        }
        ...
        public async Task<EnergyData> ReadEnergyDataAsync()
        {
            ...
            {
                _logger.LogError(ex, "Hasanoğlan IŞIKKURE Modbus okuma hatası");
                data.IsConnected = false;
        ...
        public async Task<bool> WriteCoilAsync(ushort address, bool value)
        {
            try
            {
                lock (_lock)
                {
                    if (!EnsureConnectedAsync().Result)
                    {
                        _logger.LogError("Hasanoğlan IŞIKKURE Coil yazma hatası: Bağlantı yok");
                        return false;
                    }

                    _master!.WriteSingleCoil(_settings.SlaveId, address, value);
                    _logger.LogInformation("Hasanoğlan IŞIKKURE Coil yazıldı: Address={Address}, Value={Value}", address, value);
                    return true;
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Hasanoğlan IŞIKKURE Coil yazma hatası: Address={Address}", address);
                _tcpClient?.Dispose();
                _tcpClient = null;
                return false;
            }
