...
{
    public interface IHasanoglanIpsiModbusService
    {
    ...
    public class HasanoglanIpsiModbusService : IHasanoglanIpsiModbusService, IDisposable
    {
        private readonly HasanoglanIpsiSettings _settings;
        private readonly ILogger<HasanoglanIpsiModbusService> _logger;
        ...
        public HasanoglanIpsiModbusService(ILogger<HasanoglanIpsiModbusService> logger)
        {
            _logger = logger;
            _settings = new HasanoglanIpsiSettings();
        }
        ...
        private async Task<bool> EnsureConnectedAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Hasanoğlan İPSİ Modbus bağlantısı başarılı: {Ip}:{Port}", _settings.IpAddress, _settings.Port);
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Hasanoğlan İPSİ Modbus bağlantı hatası");
                return false;
            }
        }
        ...
        public async Task<EnergyData> ReadEnergyDataAsync()
        {
            ...
            {
                _logger.LogError(ex, "Hasanoğlan İPSİ Modbus okuma hatası");
                data.IsConnected = false;
        ...
        public async Task<bool> WriteCoilAsync(ushort address, bool value)
        {
            try
            {
                lock (_lock)
                {
                    if (!EnsureConnectedAsync().Result)
                    {
                        _logger.LogError("Hasanoğlan İPSİ Coil yazma hatası: Bağlantı yok");
                        return false;
                    }

                    _master!.WriteSingleCoil(_settings.SlaveId, address, value);
                    _logger.LogInformation("Hasanoğlan İPSİ Coil yazıldı: Address={Address}, Value={Value}", address, value);
                    return true;
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Hasanoğlan İPSİ Coil yazma hatası: Address={Address}", address);
                _tcpClient?.Dispose();
                _tcpClient = null;
                return false;
            }
